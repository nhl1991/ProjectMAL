...
export default function AnimationPreviewResults({
    ...
    if (isPending) return <StatusSection><PageLoading /></StatusSection>
    if (isError) return <StatusSection><p>Something gone wrong...</p></StatusSection>
    return (
        <ResultsSection>
            {
                data.map((page, pageIndex) => {
                    ...
                    return <React.Fragment key={pageIndex}>
                        <AnimationHero category={category} value={values[pageIndex]} />
...

                                {
                                    data.map((item: AnimationData) => {
                                        ...
                                        return <SwiperSlide key={id}>
                                            <p className="absolute md:text-9xl text-3xl rankTextStroke top-2 left-2 z-20">{item.ranking ? item.ranking.rank : null}</p>
